...
from dpaygo.block import Block, BlockHeader
...
class Testcases(unittest.TestCase):
    ...
    def test_block(self, node_param):
        if node_param == "non_appbase":
            ...
            bts = self.appbase
        block = Block(self.test_block_id, dpay_instance=bts)
        ...
        block2 = Block(self.test_block_id + 1, dpay_instance=bts)
    ...
    def test_block_only_ops(self, node_param):
        if node_param == "non_appbase":
            ...
            bts = self.appbase
        block = Block(self.test_block_id, only_ops=True, dpay_instance=bts)
        self.assertEqual(block.identifier, self.test_block_id)
        self.assertTrue(isinstance(block.time(), datetime))
        self.assertTrue(isinstance(block, dict))
    ...
    def test_block_header(self, node_param):
        if node_param == "non_appbase":
            ...
            bts = self.appbase
        block = BlockHeader(self.test_block_id, dpay_instance=bts)
        self.assertEqual(block.identifier, self.test_block_id)
        self.assertTrue(isinstance(block.time(), datetime))
        self.assertTrue(isinstance(block, dict))

        block2 = BlockHeader(self.test_block_id + 1, dpay_instance=bts)
        self.assertTrue(block2.time() > block.time())
        with self.assertRaises(
            exceptions.BlockDoesNotExistsException
        ):
